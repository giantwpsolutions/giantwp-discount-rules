...
<template>
  <div class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
    <div class="bg-white rounded-lg shadow-lg h-[75vh] w-[70vw] p-6 flex flex-col">
      <!-- Modal Header -->
      <div class="border-b pb-4 mb-4 flex items-center space-x-4">
        ...
        <h3 class="text-lg font-bold">
          {{ showForm ? selectedDiscountType : __('Select Discount Type', 'aio-woodiscount') }}
        </h3>
      </div>
      ...
      <div class="flex-1 border rounded p-6 overflow-auto ">
        <template v-if="!showForm">
          <div class="grid grid-cols-1 mt-6 sm:grid-cols-2 md:grid-cols-3 gap-6">
            <!-- Flat/Percentage -->
            <div class="relative group bg-gray-100 hover:bg-blue-100 rounded-md p-4 flex flex-col items-center">
              <button
            ...
            <!-- Pro Buttons -->
            <div
              v-for="(proFeature, index) in [
                { name: 'Bulk Discount', description: 'Apply discounts based on bulk purchases' },
                { name: 'Cart Based', description: 'Discounts based on cart value' },
                { name: 'Payment Method Based', description: 'Discounts based on payment method' },
            ...
            >
              ...
              <span class="absolute top-1 right-1 bg-red-500 text-white text-xs px-2 py-1 rounded">
                Pro
              </span>
              <div class="absolute bottom-full mb-2 hidden group-hover:block bg-gray-700 text-white text-xs rounded py-1 px-2 w-48 text-center">
        ...
        <template v-else>
          ...
          <FlatPercentageForm v-if="selectedDiscountType === 'Flat/Percentage'" />
          <p v-else>{{ __('Form for', 'aio-woodiscount') }} {{ selectedDiscountType }}</p>
        </template>
      </div>
