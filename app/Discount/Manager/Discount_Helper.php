...
class Discount_Helper {
    ...
    public static function is_schedule_active( $rule ): bool {
        ...
        $start = !empty( $rule['schedule']['startDate'] ) ? strtotime( $rule['schedule']['startDate'] ) : ( ! empty( $rule['createdAt'] ) ? strtotime( $rule['createdAt'] ) : 0 );

        $end = !empty( $rule['schedule']['endDate'] ) ? strtotime( $rule['schedule']['endDate'] ) : PHP_INT_MAX; // No end date means it's always active after start
