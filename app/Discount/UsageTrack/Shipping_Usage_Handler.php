...
/**
 * Class Shipping_Usage_Handler
 *
 * Handles tracking of Shipping discount usage count.
 *
...
class Shipping_Usage_Handler
{
    ...
    /**
     * Constructor - Hooks into WooCommerce events for Shipping tracking.
     */
    ...
    /**
     * Save Shipping rule IDs from session to order meta.
     *
     * @param int $order_id WooCommerce order ID.
     */
    public function store_applied_rules_to_order($order_id)
    {
        $session_rules = WC()->session->get('_aio_shipping_applied_rules');
        ...
        if (! empty($session_rules) && is_array($session_rules)) {
            if (function_exists('aio_check_woocommerce_hpos') && aio_check_woocommerce_hpos()) {
                ...
                if ($order instanceof WC_Order) {
                    $order->update_meta_data('_aio_shipping_applied_rules', $session_rules);
                    $order->save();
                }
            } else {
                update_post_meta($order_id, '_aio_bulk_applied_rules', $session_rules);
            }
        }
    }

    /**
     * Increase usage count from Shipping rules stored in the order meta.
     *
     * @param int $order_id WooCommerce order ID.
     */
    public function increase_usage_from_order($order_id)
    {
        ...
        $applied_rules = $order->get_meta('_aio_shipping_applied_rules');
        ...
        if ($updated) {
            update_option('aio_shipping_discount', maybe_serialize($rules));
        }

        // ✅ Unset the session after processing
        if (WC()->session) {
            WC()->session->__unset('_aio_shipping_applied_rules');
        }
    }
