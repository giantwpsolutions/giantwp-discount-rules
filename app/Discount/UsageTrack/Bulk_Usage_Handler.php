...
/**
 * Class Bulk_Usage_Handler
 *
 * Handles tracking of Buy One Get One (Bulk) discount usage count.
 *
...
class Bulk_Usage_Handler
{
    ...
    /**
     * Constructor - Hooks into WooCommerce events for Bulk tracking.
     */
    ...
    /**
     * Save Bulk rule IDs from session to order meta.
     *
     * @param int $order_id WooCommerce order ID.
     */
    public function store_applied_rules_to_order($order_id)
    {
        $session_rules = WC()->session->get('_aio_bulk_applied_rules');
        ...
        if (! empty($session_rules) && is_array($session_rules)) {
            if (function_exists('aio_check_woocommerce_hpos') && aio_check_woocommerce_hpos()) {
                ...
                if ($order instanceof WC_Order) {
                    $order->update_meta_data('_aio_bulk_applied_rules', $session_rules);
                    $order->save();
                }
            } else {
    ...
    /**
     * Increase usage count from Bulk rules stored in the order meta.
     *
     * @param int $order_id WooCommerce order ID.
     */
    public function increase_usage_from_order($order_id)
    {
        ...
        $applied_rules = $order->get_meta('_aio_bulk_applied_rules');
        ...
        if ($updated) {
            update_option('aio_bulk_discount', maybe_serialize($rules));
        }

        // ✅ Unset the session after processing
        if (WC()->session) {
            WC()->session->__unset('_aio_bulk_applied_rules');
        }
    }
