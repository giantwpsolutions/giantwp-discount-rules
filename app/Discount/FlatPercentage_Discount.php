...
class FlatPercentage_Discount {
    ...
    public function maybe_apply_discount( $cart ) {
        ...
        if ( ! $matched ) {
            foreach ( $cart->get_applied_coupons() as $code ) {
                ...
                if ( $coupon->get_meta( 'gwp_is_hidden_coupon' ) ) {
    ...
    private function create_or_update_coupon( $rule, $amount ) {
        ...
        if ( ! empty( $existing ) ) {
            ...
            $coupon->update_meta_data( 'gwp_rule_id', $rule['id'] );
            $coupon->update_meta_data( 'gwp_is_hidden_coupon', true );
        }
        ...
        if ( $is_capped ) {
            $coupon->update_meta_data( 'gwp_debug_note', "Discount capped from {$calculated_discount} to {$amount}" );
        }
    ...
    public function suppress_coupon_message( $message, $message_code, $coupon ) {
        if ( $coupon instanceof \WC_Coupon && $coupon->get_meta( 'gwp_is_hidden_coupon' ) ) {
