...
class FlatPercentage_Discount {
    ...
    public function maybe_apply_discount( $cart ) {
        ...
        if ( ! $matched ) {
            foreach ( $cart->get_applied_coupons() as $code ) {
                ...
                if ( $coupon->get_meta( 'db_is_hidden_coupon' ) ) {
    ...
    private function create_or_update_coupon( $rule, $amount ) {
        ...
        if ( ! empty( $existing ) ) {
            ...
            $coupon->set_description( __('Auto-generated by DealBuilder Discount Rules', 'dealbuilder-discount-rules') );
            $coupon->update_meta_data( 'db_rule_id', $rule['id'] );
            $coupon->update_meta_data( 'db_is_hidden_coupon', true );
        }
        ...
        if ( $is_capped ) {
            $coupon->update_meta_data( 'db_debug_note', "Discount capped from {$calculated_discount} to {$amount}" );
        }
    ...
    private function get_discount_rules(): array {
        return maybe_unserialize( get_option( 'dealbuilder_flatpercentage_discount', [] ) ) ?: [];
    }
    ...
    public function suppress_coupon_message( $message, $message_code, $coupon ) {
        if ( $coupon instanceof \WC_Coupon && $coupon->get_meta( 'db_is_hidden_coupon' ) ) {
